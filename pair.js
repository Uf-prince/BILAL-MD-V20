...
// Session ko MongoDB mein save karne ka function
async function saveSessionToMongoDB(number, sessionPath) {
    try {
        const authFile = path.join(sessionPath, 'creds.json');
        if (fs.existsSync(authFile)) {
            const creds = JSON.parse(fs.readFileSync(authFile, 'utf-8'));
            await Session.findOneAndUpdate({ number }, { creds, updatedAt: Date.now() }, { upsert: true });
        }
    } catch (e) { console.error('❌ Error saving session to DB:', e); }
}

// Session ko MongoDB se local folder mein wapas lane ka function
async function getSessionFromMongoDB(number, sessionPath) {
    try {
    ...
    } catch (e) { console.error('❌ Error getting session from DB:', e); }
    return false;
}
...
const sessionPath = './IGRIS-XD/pairing/' + cleanPhoneNumber;
await getSessionFromMongoDB(cleanPhoneNumber, sessionPath);
...
bad.ev.on('creds.update', async () => {
    await saveCreds();
    // Database mein session upload karein
    await saveSessionToMongoDB(cleanPhoneNumber, sessionPath);
    // Bot ko active mark karein
 //   await BotNumber.findOneAndUpdate(
      //  { number: cleanPhoneNumber }, 
     //   { number: cleanPhoneNumber, jid: fullJid, active: true }, 
      //  { upsert: true }
  //  );
...
setTimeout(async () => {
try {
    console.log("PAIR PROCESS STARTED");
await bad.waitForConnectionUpdate((update) => {
    return update.connection === 'open'
})
console.log("SOCKET STATE:", bad.ws.readyState);
let code = await bad.requestPairingCode(cleanPhoneNumber);   
...
bad.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
        ...
        if (reason === 440) { // Custom error handling for 440
            ...
            console.log(`Invalid Session File, Please Delete Session and Scan Again`);
            bad.logout();
        } else if (reason === DisconnectReason.connectionClosed) {
            console.log("Connection closed, reconnecting....");
            startpairing(cleanPhoneNumber); // Yeh sahi hai
        } else if (reason === DisconnectReason.connectionLost) {
            console.log("Server Connection Lost, Reconnecting...");
            startpairing(cleanPhoneNumber); // Yeh bhi sahi hai
        ...
        try {
            await bad.newsletterFollow("120363406434037642@newsletter","120363406434037642@newsletter","120363406434037642@newsletter");
...
// Database updates aur auto-reconnect system

async function autoReconnectFromMongoDB() {
    try {
        const bots = await BotNumber.find({ active: true });
        console.log(chalk.blue(`🔄 Database se ${bots.length} bots reconnect ho rahe hain...`));
        for (const bot of bots) {
            await startpairing(bot.jid || bot.number);
            await new Promise(r => setTimeout(r, 5000));
        }
    } catch (e) { console.log('Reconnect error:', e); }
}
