...
global.channelLink = "https://whatsapp.com/channel/0029VbBRSOALikg4s32Dp70c";
global.ytch = "META";
...
// Database connect hote hi bots ko online karne ka system
setTimeout(async () => {
    try {
        if (typeof autoReconnectFromMongoDB === 'function') {
            console.log('✨ BILAL-MD: Reconnecting bots from database...');
            await autoReconnectFromMongoDB();
        }
    } catch (err) {
        console.error('❌ Auto-reconnect error:', err);
    }
}, 10000); // 10 second ka wait taaki main connection stable ho jaye
